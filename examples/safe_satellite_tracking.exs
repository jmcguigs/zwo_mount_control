...
IO.puts("   ⚠️  Please ensure mount has clear path to home position")
...
try do
  ...
  |> Enum.reduce_while(:ok, fn _, _ ->
    ...
    if elapsed >= 60 do
      ...
      if mnt.alt < 0.0 do
        IO.puts("\n⚠️  SAFETY ABORT: Mount altitude #{Float.round(mnt.alt, 1)}° is below horizon!")
        {:halt, :safety_abort}
      else
